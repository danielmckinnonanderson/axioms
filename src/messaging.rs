...
impl TryFrom<Vec<u8>> for MessageType {
    ...
    fn try_from(value: Vec<u8>) -> Result<Self, Self::Error> {
        ...
        fn vec_to_slice(vec: Vec<u8>) -> Result<[u8; 6], ()> {
            if vec.len() != 6 {
                return Err(());
            }
        ...
        if let Some(byte) = iter.next() {
            match byte {
                0x00 => {
                    ...
                    return Ok(MessageType::ClientReadyStatus(ClientReadyPayload{ ready: payload }))
                },
                0x01 => return Ok(MessageType::GameInit),
                0x02 => {
                    ...
                    let slice = vec_to_slice(avail)?;

                    return Ok(MessageType::RoundInit(RoundInitPayload {
...
mod tests {
    ...
    fn test_msg_bin_serialization() {
        let cl_ready: Vec<u8> = MessageType::ClientReadyStatus(ClientReadyPayload{ ready: true }).into();
    ...
    fn test_msg_bin_deserialization() {
        ...
        assert_eq!(cl_ready_0, MessageType::ClientReadyStatus(ClientReadyPayload { ready: false }));
        let cl_ready_1: MessageType = vec![0x00, 0x01].try_into().unwrap();
        assert_eq!(cl_ready_1, MessageType::ClientReadyStatus(ClientReadyPayload { ready: true }));

        let gm_init: MessageType = vec![0x01].try_into().unwrap();
        assert_eq!(gm_init, MessageType::GameInit);

        let rd_init: MessageType = vec![0x02, 0x01, 0x03, 0x03, 0x05, 0x01, 0x07].try_into().unwrap();
        assert_eq!(rd_init, MessageType::RoundInit(RoundInitPayload {
